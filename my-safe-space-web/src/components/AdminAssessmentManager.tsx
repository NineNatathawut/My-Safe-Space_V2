  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-800">📊 จัดการแบบประเมิน</h2>
          <p className="text-sm text-slate-500 mt-0.5">รายการแบบประเมินทั้งหมดในระบบ</p>
        </div>
        <button
          onClick={() => navigate('/admin/assessments/create')}
          className="flex items-center gap-2 bg-gradient-to-r from-fuchsia-500 to-purple-600 text-white px-5 py-2.5 rounded-xl font-bold text-sm hover:shadow-md transition-all"
        >
          <span>➕</span>
          สร้างแบบประเมินใหม่
        </button>
      ...
      {assessments.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center">
          ...
          <h3 className="text-lg font-bold text-slate-700 mb-1">ยังไม่มีแบบประเมิน</h3>
          <p className="text-sm text-slate-400 mb-6">กดปุ่ม "สร้างแบบประเมินใหม่" เพื่อเริ่มต้น</p>
          <button
            onClick={() => navigate('/admin/assessments/create')}
            className="inline-flex items-center gap-2 bg-gradient-to-r from-purple-700 to-fuchsia-600 text-white px-6 py-2.5 rounded-xl font-medium text-sm hover:from-purple-800 hover:to-fuchsia-700 transition-colors"
          >
            ➕ สร้างแบบประเมินใหม่
          </button>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/80">
                  <th className="text-left py-3.5 px-4 text-slate-500 font-semibold">ชื่อแบบประเมิน</th>
                  <th className="text-left py-3.5 px-4 text-slate-500 font-semibold">ประเภท</th>
                  <th className="text-left py-3.5 px-4 text-slate-500 font-semibold">สถานะ</th>
                  <th className="text-center py-3.5 px-4 text-slate-500 font-semibold">ข้อสอบ</th>
                  <th className="text-left py-3.5 px-4 text-slate-500 font-semibold">วันที่สร้าง</th>
                  <th className="text-right py-3.5 px-4 text-slate-500 font-semibold">จัดการ</th>
                </tr>
              </thead>
              <tbody>
                {assessments.map((a) => (
                  <tr key={a.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      <span className={`inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-lg ${
                        a.type === 'INTERNAL' ? 'bg-purple-50 text-purple-700' : 'bg-amber-50 text-amber-700'
                      }`}>
      ...
      <details className="bg-white rounded-2xl border border-slate-100 shadow-sm group">
        <summary className="flex items-center justify-between p-5 cursor-pointer select-none list-none">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-700">ตั้งค่าเพิ่มเติม</span>
            <span className="text-xs text-slate-400">(Pre/Post Toggle & นำเข้าไฟล์)</span>
          </div>
        ...
        <div className="px-5 pb-5 space-y-6 border-t border-slate-100 pt-5">
          ...
          {activeAssessment ? (
            <div className="bg-slate-50 rounded-xl p-5 border border-slate-200/80">
              <h3 className="font-semibold text-slate-700 mb-1">
                ชุดแบบประเมินปัจจุบัน: <span className="text-purple-600">{activeAssessment.title}</span>
              </h3>
              <p className="text-xs text-slate-500 mb-4">เปิด/ปิด การแสดงแบบประเมินก่อนและหลังใช้งาน</p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center justify-between bg-white p-4 rounded-lg shadow-sm border">
                  <div>
                    <span className="font-medium text-slate-800 block">Pre-Assessment</span>
                    <span className="text-xs text-slate-500">แบบประเมินก่อนเริ่มใช้งาน</span>
                  </div>
                  <button
                    onClick={() => handleToggle('pre', activeAssessment.is_active_pre)}
                    disabled={savingToggle}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                      activeAssessment.is_active_pre ? 'bg-emerald-500' : 'bg-slate-300'
                    } disabled:opacity-50`}
                  >
                    <span
                ...
                <div className="flex items-center justify-between bg-white p-4 rounded-lg shadow-sm border">
                  <div>
                    <span className="font-medium text-slate-800 block">Post-Assessment</span>
                    <span className="text-xs text-slate-500">แบบประเมินหลังใช้งาน</span>
                  </div>
                  <button
                    onClick={() => handleToggle('post', activeAssessment.is_active_post)}
                    disabled={savingToggle}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                      activeAssessment.is_active_post ? 'bg-emerald-500' : 'bg-slate-300'
                    } disabled:opacity-50`}
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
            ...
            <div className="text-center py-6 text-slate-400 text-sm bg-slate-50 rounded-xl border border-slate-200/80">
          ...
          <div className="space-y-4">
            <h3 className="font-semibold text-slate-800">📥 นำเข้าชุดแบบประเมินใหม่ (JSON / Excel)</h3>
            <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center hover:border-purple-400 transition-colors bg-slate-50/50">
              <input
                type="file"
              ...
              <label htmlFor="file-upload" className="cursor-pointer flex flex-col items-center">
                ...
                <span className="text-sm font-medium text-purple-600">คลิกเพื่อเลือกไฟล์ .json หรือ .xlsx</span>
                <span className="text-xs text-slate-400 mt-1">รองรับโครงสร้างแบบประเมินมาตรฐาน</span>
              </label>
            </div>
            ...
            {importedData && (
              <div className="bg-purple-50/50 p-4 rounded-xl border border-purple-100 space-y-3">
                <div className="flex justify-between items-center">
                  <h4 className="font-bold text-purple-900">🔍 ตัวอย่างข้อมูลที่อ่านได้:</h4>
                ...
                <p className="text-sm text-slate-700"><strong>ชื่อ:</strong> {importedData.title}</p>
                <p className="text-sm text-slate-600"><strong>คำอธิบาย:</strong> {importedData.description || '-'}</p>
                <div className="pt-2 space-y-1">
                  <label className="block text-xs font-semibold text-slate-700">🔗 แนบลิงก์เพิ่มเติม (ถ้ามี):</label>
                  <input
                    type="url"
                    placeholder="https://example.com/more-info"
                    value={(importedData as any).link_url || ''}
                    onChange={(e) => setImportedData({ ...importedData, link_url: e.target.value } as any)}
                    className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:border-purple-500 transition-colors"
                  />
                </div>
                <button
                  onClick={handleSaveImport}
                  disabled={savingImport}
                  className="w-full mt-2 bg-purple-600 hover:bg-purple-700 text-white font-medium py-2.5 px-4 rounded-lg transition-colors shadow-sm disabled:opacity-50"
                >
