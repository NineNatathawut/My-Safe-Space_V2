...
export function PostComposerModal({ onClose }: PostComposerModalProps) {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-ink-deep/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
        <div className="bg-white rounded-3xl max-w-xl w-full shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
          <div className="flex items-center justify-between px-6 py-4 border-b border-hairline">
            <h2 className="font-feather text-lg font-extrabold text-ink">พื้นที่แบ่งปันเรื่องราว</h2>
            <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-owl-soft text-body-soft hover:text-body-strong transition-colors">✕</button>
          </div>
          ...
          <div className="p-6 max-h-[70vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                <h3 className="font-bold text-body-strong mb-3">บอกเล่าสิ่งที่อยู่ในใจ...</h3>
                <textarea
                  className="input min-h-[160px] resize-none disabled:opacity-50 disabled:bg-owl-soft/30 text-sm"
